...
## 2.5. Fonte única de env + build/deploy limpo do zero (sem stale)
Verifique (`references/ops.md` §3):
- **Config de fonte única:** toda env vem de **um lugar só** — `/<app>/.env` global (self-host) ou as **env vars do projeto** no provedor (host gerenciado). `.env` solto no servidor, valor cravado à mão divergindo do declarado, ou constante de config hardcoded no código são **achados**.
- **Todo (re)deploy é build LIMPO do zero** a partir de git + env: sem reaproveitar `.next`/`dist`/`node_modules` sujo, sem patch por cima do site publicado. Cache só se **endereçado por hash/invalidável**. Prove reprodutibilidade: mesmo commit + env → mesmo output.
- **Dados preservados:** rebuildar **nunca** zera CMS/banco/uploads/conteúdo (só mudam pelo fluxo próprio). Reset de dado é operação **separada e gated** (dev/hml), nunca efeito de publicar.

## 2.6. Isolamento por app
Verifique (`references/ops.md` §4):
- **Self-host:** **um user Linux + systemd/container hardened por app** (`User=`, `NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome`, `PrivateTmp`, `ReadWritePaths` mínimo). Multi-app self-hosted: **isolamento por usuário** — nunca dois apps no mesmo user, nunca `root`; user/unit provisionados pelo pipeline/ops, nunca à mão.
- **Host gerenciado (Vercel/Netlify/CDN):** o isolamento é do provedor (deploy imutável, sandbox). O piso do seu lado é **não furar** a sandbox (segredo só em env do servidor, função serverless/edge sem vazar credencial).
...
## 4. Saída
Grave o plano/relatório em `<projeto>_archive/` (§28): estado do fluxo de ambientes, cobertura do pipeline (o que ainda é manual), e — em multi-app — tempo de build, grau de paralelismo e qualquer colisão de independência (com a correção priorizada). Confirme ao usuário: fluxo ok? deploy/rollback só pelo pipeline (nada direto no site)? promoção operável sem a IA? config de **fonte única** e (re)deploy é **build limpo do zero** (sem stale, dados preservados)? **isolamento por app** (self-host: user+unit hardened; gerenciado: sandbox do provedor)? multi-app com build paralelo em `nproc` e independência provada?
