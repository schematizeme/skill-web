...
1. **Leia agora, na íntegra, TODOS os arquivos** de references da skill — não trabalhe de memória, abra cada arquivo. O caminho é `.claude/skills/schematize-web/references/*.md` (instalação no projeto) ou `~/.claude/skills/schematize-web/references/*.md` (instalação global). Com destaque para:
   - `padroes-codigo.md`/`qualidade.md` — **clean code**: tamanho em camadas (teto DURO 750/arquivo, ~500 úteis, que bloqueia; flag em >300 úteis que sempre sinaliza), micro-componentes/hooks, doc-comment, índice, DoD.
   ...
   - `ops.md` — **ambientes e deploy pelo pipeline**: fluxo dev→local→github→hml/preview→prd (nada direto no site/servidor deployado, artefato imutável com commit SHA), pipeline de CI/CD como interface única de deploy/rollback (promoção sem depender da IA); em multi-app, build paralelo=`nproc` e independência=invariante.
   - `dados-estado.md`, `design-tokens.md`, `design-referencias.md`, `operacao-deploy.md`, `observabilidade.md`, `testes.md`, `anti-padroes.md`, `stack-versoes.md`, `contexto-claude-code.md`.
