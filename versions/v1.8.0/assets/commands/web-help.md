...
| `/web-review` | Roda o gate da Definition of Done e dos anti-padrões (§35, §37): tamanho de arquivo em camadas (teto 750 bloqueia, >300 úteis flagueia), componente/hook sem doc-comment, índice desatualizado, macaquices de segurança/a11y/perf. |
| `/web-index` | (Re)gera o índice de componentes/hooks (§39) a partir dos doc-comments. |
| `/web-ops` | Verifica o **fluxo de ambientes**/preview (dev→local→github→hml/preview→prd, nada direto no site/servidor deployado), o **deploy só pelo pipeline** (interface única, promoção sem depender da IA) e, em projeto **multi-app**, o build paralelo (`nproc`) e a independência. |
