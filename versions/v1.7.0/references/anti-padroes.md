...
36. **Editar direto no site/servidor deployado** (hml/prd) — mexer no build já publicado, hotpatch de HTML/JS/CSS servido pelo CDN/host, trocar asset "na mão" no bucket/edge pra "resolver rápido".
    → O site no ar é **imutável por edição manual**; recebe só **artefato promovido do git** (commit SHA). Correção segue o fluxo, acelerada (`references/ops.md` §1, §28.1 de `qualidade.md`).
37. **Subir mudança direto pra hml/preview ou prd** pulando `dev local → teste local → GitHub`.
    → Fluxo de promoção é fixo e sem atalho: nada chega a hml/prd sem passar por git + teste local (`references/ops.md` §1).
38. **Deploy manual ad-hoc fora do pipeline** (`scp`/`rsync`/upload do build, "arrastar `dist/` pro painel", subir asset solto no CDN, rodar deploy da máquina de alguém por fora).
    → Build/publicação/promoção/rollback passam pelo **pipeline de CI/CD** — a interface única. Não tem etapa pra aquilo? **adiciona no pipeline**, não faz por fora; a promoção tem que ser operável pelo usuário sem a IA (`references/ops.md` §2).
39. **Deploy que reaproveita build/cache/artefato stale em vez de rebuildar limpo do zero** — reusar `.next`/`dist`/`node_modules` sujo de build anterior, ou publicar patch incremental por cima do que já está no ar, em vez de recriar o build a partir do commit + env.
    → Todo (re)deploy é **build limpo do zero** a partir de git + env (cache só endereçado por hash/invalidável), determinístico e reprodutível; **preserva os dados** — CMS/banco/uploads nunca zerados por um redeploy (`references/ops.md` §3).
40. **Config/env de ambiente espalhada fora da fonte única** — `.env` solto no servidor, valor cravado à mão no painel do host divergindo do declarado, constante de config hardcoded no código.
    → **Fonte única de env**: `/<app>/.env` global (self-host) ou env vars do projeto no provedor (host gerenciado); segredo via secret manager, nunca no bundle (`references/ops.md` §3, §43.1).
41. **Quebrar o isolamento por app** — no self-host, dois apps no mesmo user Linux, app rodando como `root`, ou user/unit criado à mão; no host gerenciado, furar a sandbox (credencial/segredo vazando da função serverless/edge, escrita fora do escopo do deploy).
    → **Isolamento por app**: self-host = user + systemd/container hardened por app (nunca `root`, nunca dois no mesmo user), provisionado pelo pipeline/ops; gerenciado = respeitar a sandbox imutável do provedor (`references/ops.md` §4).

### Design e refino visual

42. **Emoji na estrutura do site** (nav/botão/card/feature/bullet/título/empty state/status/seta) no lugar de ícone — sem o usuário ter pedido.
    → **Um icon set** (Lucide/Phosphor/Heroicons/Radix), SVG com `currentColor`, tamanho por token, `aria-hidden` quando decorativo. Emoji renderiza diferente por SO, não herda cor/peso (quebra tema/dark) e destoa da marca (`references/design-refino.md`, regra zero). Emoji só quando é conteúdo pedido (reação, UGC, post sobre emoji).
43. **Misturar icon sets** (Lucide + Font Awesome + emoji na mesma tela) ou `stroke-width`/grade de ícone inconsistentes.
    → Uma família só, mesmo `stroke`, mesma grade (20/24px). Falta o ícone no set? escolhe outro conceito do set ou adiciona o SVG — nunca tapa com emoji (`design-refino.md`).
44. **Front "coeso" mas amador** — tokens no lugar, mas espaçamento apertado/uniforme, hierarquia tímida (tudo do mesmo tamanho), acento espalhado em tudo, borda grossa ou sombra dura, texto full-width, sem estados de hover/foco, empty/loading/erro crus.
    → Coesão evita o site *quebrado*; refino faz parecer *desenhado*. Aplique as **13 alavancas** e rode o **checklist de reprovação** (`references/design-refino.md`) — os quatro maiores: **espaço, hierarquia, um acento, ícone-não-emoji**. Refino é subtração, dentro do budget de CWV (§45).

> Regra de bolso: se a justificativa começa com "só pra funcionar", "depois eu arrumo" ou "é mais rápido assim" e o resultado mexe em segredo, auth, acessibilidade, performance, registro — **ou toca o site/servidor por fora do fluxo/pipeline, reaproveita build stale, espalha config fora da fonte única, ou entope a tela de emoji e efeito no lugar de espaço e hierarquia** — **provavelmente é uma macaquice desta lista. Para e faz certo.**
