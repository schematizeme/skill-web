...
- **Macro** → `<projeto>_archive/index/INDEX_GLOBAL.md`: `mermaid flowchart` de **rotas → páginas → componentes** e o **consumo de APIs/BFF**.
- **Componentes** → `<projeto>_archive/index/INDEX_COMPONENTS.md` e `<projeto>_archive/index/MAPA.md` (§5): `mermaid flowchart` de quem **renderiza/usa** quem **+** adjacência textual (`A -> B`). Cada componente/hook é um nó.
...
- `<projeto>_archive/index/INDEX_GLOBAL.md`: **cada** app/área com 1 linha + o grafo macro. Espelhe no `<projeto>_archive/index/MAPA.md` (§4/§5) — **no archive, nunca no root**.
